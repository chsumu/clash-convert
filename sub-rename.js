/**
 * 更新日期：2026-04-23
 * 原地址：https://raw.githubusercontent.com/Keywos/rule/main/rename.js
 * 修改说明：source源前缀
 * 
 * 用法：Sub-Store 脚本操作添加
 ...
 *
 *** 前缀参数
 * [source]  输出节点名前添加source源前缀
 * [name=]  节点添加机场名称前缀；
 * [nf]     把 name= 的前缀值放在最前面
 *** 保留参数
 * [blkey=iplc+gpt+NF+IPLC] 用+号添加多个关键词 保留节点名的自定义字段 需要区分大小写!
 * 如果需要修改 保留的关键词 替换成别的 可以用 > 分割 例如 [#blkey=GPT>新名字+其他关键词] 这将把【GPT】替换成【新名字】
 * 例如      https://raw.githubusercontent.com/chsumu/clash-convert/refs/heads/main/sub-rename.js#blgd&flag&source&blkey=AI>AI
 * [blgd]   保留: 家宽 IPLC ˣ² 等
...
function operator(pro) {
    ...
    pro.forEach((e) => {
        ...
        if (findKey?.[1]) {
            ...
            let subSource = "";
            if (addSource) {
                // 优先检测用户确认的 _subName 属性
                let sName = e._subName || e.source || e._source || e.subName || e.sub || e.tag || (e.tags && e.tags[0]) || "";
                if (sName) subSource = `[${sName}]`;
            }
            keyover = keyover
                .concat(subSource, firstName, usflag, nNames, findKeyValue, retainKey, ikey, ikeys)
                .filter((k) => k !== "");
